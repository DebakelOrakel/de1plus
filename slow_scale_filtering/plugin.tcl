...
namespace eval ::plugins::${plugin_name} {
	...
	variable description "Provides slow filtering for display for many skins"
	...
	proc main {} {
		...
		# 41 samples (40 intervals) is about 4 seconds, for about 2 seconds of delay
		# This is similar to the 20-sample tau of the old estimator

		variable flow_samples_slow 41

		# Need at least two samples to compute a difference
		if { $flow_samples_slow < 2 } { set flow_samples_slow 2 }

		# Reasonable limit of 10-second window
		if { $flow_samples_slow > 101 } { set flow_samples_slow 101 }
		...
		proc install_slow_estimators {event_dict} {
			...
			msg -NOTICE "slow_scale_filtering: Installed slow filtering for mass-flow"
		}
		...
		# In case the scale has already connected
		# An empty event_dict is "safe" by inspection of the code above

		::plugins::slow_scale_filtering::install_slow_estimators {}
